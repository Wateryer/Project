    ...
    public class ThreadManager : Manager {
        ...
        private void ProgressChanged(object sender, DownloadProgressChangedEventArgs e) {
            ...
            */
            ...
            value = string.Format("{0}",Math.Round (e.BytesReceived / 1024d ,2));
            data = new NotiData(NotiConst.UPDATE_COUNTLENGTH, value);
            if (m_SyncEvent != null) m_SyncEvent(data);
            if (e.ProgressPercentage == 100 && e.BytesReceived == e.TotalBytesToReceive) {
                sw.Reset();

                data = new NotiData(NotiConst.UPDATE_FINISH_ONE, currDownFile);
                //data = new NotiData(NotiConst.UPDATE_DOWNLOAD, 0);
                if (m_SyncEvent != null) m_SyncEvent(data);

                data = new NotiData(NotiConst.UPDATE_CUR_COUNTLENGTH, value);
                if (m_SyncEvent != null) m_SyncEvent(data);
            }
        }
        ...
        void OnExtractFile(List<object> evParams) {
            ...
            ///------------------通知更新面板解压完成--------------------
            NotiData data = new NotiData(NotiConst.UPDATE_FINISH_ONE, null);
            if (m_SyncEvent != null) m_SyncEvent(data);
        }
